...
import { Modal, ModalFoot, ModalHead, NumberField, RadioRow } from '../primitives';
...
export const NewRoomModal = () => {
  ...
  const nr = state.nr;
  const src = nr.from ? p.rooms.find((r) => r.code === nr.from) : undefined;

  return (
    <Modal
      width={520}
      panelStyle={{ maxHeight: 'calc(100vh - 60px)', display: 'flex', flexDirection: 'column' }}
      onClose={() => dispatch({ type: 'CLOSE_NEW_ROOM' })}
    >
      <ModalHead
        title={src ? `${src.name} 복제해서 만들기` : '객실 만들기'}
        sub={src ? '값·구조·침구를 그대로 가져왔습니다. 다른 것만 고치세요.' : '건드리지 않은 항목은 숙소 전체값을 그대로 씁니다.'}
      />
      ...
      <div style={{ padding: '16px 18px', overflowY: 'auto', minHeight: 0 }}>
        <RoomInfoFields
          p={p}
          value={nr}
          floorText={nr.floorText}
          onChange={(patch) => dispatch({ type: 'SET_NR', patch })}
          onFloorText={(v) => dispatch({ type: 'SET_NR', patch: { floorText: v } })}
        />
